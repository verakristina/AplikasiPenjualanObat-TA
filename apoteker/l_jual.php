...
<?php $ab = mysqli_query($koneksi,"select * from detail_transaksi, obat where obat.kode_obat = detail_transaksi.kode_obat and id_transaksi='$a'") ?>
    ...
    <?php 
      ...
      <?php $ad = mysqli_query($koneksi,"select count(id_transaksi) as jumlah_obat from detail_transaksi where id_detailTransaksi='$a' ")?>
      <tr bgcolor="#DCDCDC">
        <td>Total Item</td>
        <?php while ($dy = mysqli_fetch_array($ad)) { ?>
        <td>: <?php echo $dy['jumlah_obat'] ?> Item</td>
      <?php } ?>
      </tr>
