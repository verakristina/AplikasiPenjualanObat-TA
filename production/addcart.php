...
if ($qty <= $stok) :
    if ($total==0 ) {
       ...
       $query5 = mysqli_query($koneksi,"UPDATE obat SET stok='$updatestok' WHERE kode_obat='$kode12'");
    } else {
        $update = $qty+$upqty;
       $query4 = mysqli_query($koneksi,"UPDATE temp SET qty='$update', subtotal='$bayar' WHERE kode_obat='$kode12' ");
       $query5 = mysqli_query($koneksi,"UPDATE obat SET stok='$updatestok' WHERE kode_obat='$kode12'");
    //
